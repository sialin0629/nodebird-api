...
router.post('/domain', isLoggedIn, createDomain); // /domain 경로에 POST 요청 시, isLoggedIn 미들웨어 실행 후 createDoamin 함수 실행
// createDomain: 도메인 생성을 처리하는 함수로, POST 요청 시 필요한 비즈니스 로직을 수행
// isLoggedIn: 사용자가 인증되었는지 확인하는 미들우에ㅓ, 인증되지 않았으면 오류를 반황하거나 로그인 페이지로 리디렉션함.
